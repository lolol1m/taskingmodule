...
# Toggle env/config:Uncomment this and comment config=ConfigClass() when using config and not env var
...
config = load_config(env_mode=True) # Comment this to not read by env vars
