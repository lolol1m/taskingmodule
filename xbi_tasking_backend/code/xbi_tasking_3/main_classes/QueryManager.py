...
class QueryManager():
    ...
    def completeImage(self, scvu_image_id, vetter, current_datetime):
        ...
        Input:      scvu image id, current_datetime
        Output:     NIL
        '''
        
        query = f"UPDATE image SET completed_date = %s, vetter_id = (SELECT id FROM users WHERE name = %s) WHERE scvu_image_id = %s"
        cursor = self.db.executeUpdate(query, (current_datetime, vetter, scvu_image_id))
    ...
    def getAssigneeID(self, assignee):
        ...
        cursor = self.db.executeSelect(query, (assignee, ))
        if len(cursor) == 0:
            return None
        return cursor
